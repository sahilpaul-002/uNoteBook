...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`navbar navbar-expand-lg ${theme === "light" ? "bg-warning-subtle" : "bg-dark"} fixed-top`} data-bs-theme={theme === "light" ? "light" : "dark"} style={{ ...(theme === "light" ? { backgroundColor: "#e3f2fd", boxShadow: "0 10px 50px rgba(90, 90, 90, 0.6)" } : { boxShadow: "0 10px 50px rgba(136, 134, 134, 0.6)" }) }}>
        <div className="container-fluid">
          ...
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            {localStorage.getItem("loginToken") !== null ?
              <Link className={`link-secondary link-offset-2 link-underline link-underline-opacity-0 ${isCollapsed ? "float-start my-2 mx-2" : "ms-4 me-3"}`} aria-current="page" onClick={handleOnLogOut}>Log Out</Link> :
              <>
                <Link className={`${location.pathname === "/signup" ? "link-dark" : "link-secondary"} link-offset-2 link-underline link-underline-opacity-0 ${isCollapsed ? "float-start my-2 mx-2" : "ms-4 me-1"}`} aria-current="page" to="/signup">Sign Up</Link>
                <Link className={`${location.pathname === "/login" ? "link-dark" : "link-secondary"} link-offset-2 link-underline link-underline-opacity-0 ${isCollapsed ? "float-start my-2 mx-2" : "ms-1 me-3"}`} aria-current="page" to="/login">Log In</Link>
              </>
            }
